...
- Use low-fidelity, neutral boxes, lines, simple symbols, and legible text.
  ...
  scrolling boundaries, and primary/secondary action priority.
...
actions, such as empty content, validation errors, or permission denial. Do
not invent states just to fill a checklist. Text may cover changes that do
...
Check region order, content, action placement, relative sizing, and state
against the canonical description. Check for clipped labels, overlaps,
unreadable text, unintended platform styling, and invented controls.
